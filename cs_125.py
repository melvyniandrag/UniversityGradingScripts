"""
Script for computing final grades in cs_125 class.
The script takes a CSV of grade data downloaded from blackboard and computes the final grade for each student.
To test it out, run

	python3 cs_125.py cs125-example.data

The script outputs a csv called $OUTPUT_CSV_NAME ( see variables below) and also dumps the average  out to the command line for quick inspection.
"""
...
EXAM_WEIGHT = 0.4
HOMEWORK_WEIGHT = 0.6
OUTPUT_CSV_NAME = "final_grades_125.csv"
NUM_EXTRA_CREDITS = 0
DROP_THIS_MANY_HOMEWORKS = 3
TOTAL_HOMEWORKS_TO_DROP = NUM_EXTRA_CREDITS + DROP_THIS_MANY_HOMEWORKS
HOMEWORK_EXTRA_CREDIT_REGEX = "(Homework|Extra)"
...
def compute_student_hw_average(row):
    ...
    """
    print("called compute_student_hw_average with data: " + str(row))
    homeworks = row.filter(regex=HOMEWORK_EXTRA_CREDIT_REGEX)
...
def number_to_letter(row):
    ...
    - first take the final grade and divide by 25 to get it in the [0,4.0] range instead of [0,100]
...
if __name__ == "__main__":
    ...
    df['final'] = HOMEWORK_WEIGHT * df["hw_average"] + EXAM_WEIGHT * df["exam_average"] # df.apply(compute_student_grade, axis=1)
